      ...
      <div className="wrap" style={{position:'relative', zIndex:1}}>
        ...
        <div style={mapStyles.layout}>
          ...
          <div style={mapStyles.mapBox}>
            ...
            {/* Fixed-aspect stage: aspect matches the silhouette PNG (736:1024) */}
            <div style={{
              position:'absolute',
              top:'50%', left:'50%',
              transform:'translate(-50%, -50%)',
              width:'calc(100% - 40px)',
              aspectRatio:'736 / 1024',
            }}>
              {/* Peru silhouette — real image as authoritative outline */}
              <img src="assets/peru-silhouette.png" alt="" aria-hidden="true"
                style={{
                  position:'absolute',
                  inset:0,
                  ...
                  pointerEvents:'none',
                  opacity:0.95,
                  filter:'drop-shadow(0 0 12px rgba(201,169,97,0.12))',
                  zIndex:1,
                }}
              />

              <svg viewBox="0 0 100 140" style={{position:'absolute', inset:0, width:'100%', height:'100%', zIndex:2}} preserveAspectRatio="none">
              ...
              {/* ══════ 25 clickable region hit-zones ══════
                  Invisible by default; fill on hover/selection to highlight the department */}
              {REGIONS.map(r => {
                const d = window.PERU_SHAPES?.[r.id];
                if (!d) return null;
                ...
                return (
                  <path key={`shape-${r.id}`} d={d}
                    fill={color}
                    fillOpacity={isSel ? 0.28 : 0}
                    stroke={isSel ? color : 'transparent'}
                    strokeWidth={isSel ? '0.4' : '0'}
                    style={{cursor:'pointer', transition:'fill-opacity .3s, stroke .3s'}}
                    onClick={() => setSelected(r.id)}
                    onMouseEnter={(e) => { if (!isSel && !dim) { e.currentTarget.setAttribute('fill-opacity','0.14'); e.currentTarget.setAttribute('stroke', color); e.currentTarget.setAttribute('stroke-width','0.25'); } }}
                    onMouseLeave={(e) => { if (!isSel) { e.currentTarget.setAttribute('fill-opacity','0'); e.currentTarget.setAttribute('stroke','transparent'); } }}
                  />
                );
              })}
              ...
              {REGIONS.map(r => {
                ...
                return (
                  <g key={r.id} style={{cursor:'pointer', opacity: dim ? 0.2 : 1, transition:'opacity .3s'}}
                     onClick={() => setSelected(r.id)}>
                    {isSel && (
                      <circle cx={cx} cy={cy} r="3" fill="none" stroke={color} strokeWidth="0.3" opacity="0.5">
                        <animate attributeName="r" values="3;6;3" dur="2s" repeatCount="indefinite" />
                        <animate attributeName="opacity" values="0.6;0;0.6" dur="2s" repeatCount="indefinite" />
                      </circle>
                    )}
                    <circle cx={cx} cy={cy} r={isSel ? 1.5 : 0.9} fill={color}
